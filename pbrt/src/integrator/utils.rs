...
pub fn uniform_sample_all_lights<'a>(isect: &(impl Into<Interactions<'a>> + Clone), scene: &Scene, sampler: &mut Box<Sampler + Send>, arena: &(), n_samples: &[u32], handle_media: bool) -> Spectrum {
...
pub fn uniform_sample_one_light<'a>(isect: &(impl Into<Interactions<'a>> + Clone), scene: &Scene, sampler: &mut Box<Sampler + Send>, arena: &(), n_samples: &[u32], handle_media: bool) -> Spectrum {
...
pub fn estimate_direct<'a>(isect: &(impl Into<Interactions<'a>> + Clone), u_scattering: Point2f, light: &Arc<Light + Send + Sync>, u_light: Point2f, scene: &Scene, sampler: &mut Box<Sampler + Send>, arena: &(), handle_media: bool, specular: bool) -> Spectrum {
    ...
    if light_sample.pdf > 0.0 && !light_sample.li.is_black() {
        ...
        if vis.is_some() && !f.is_black() {
            ...
            if handle_media {
                light_sample.li *= vis.tr(scene, sampler.as_ref());
    ...
    if !light.is_delta_light() {
        ...
        if let Some(isect) = isect.get_surface() {
            if let Some(bsdf) = &isect.bsdf {
                if let Some(sample) = bsdf.sample_f(isect.wo, u_scattering) {
